...
const ejerciciosIniciales: Ejercicio[] = [
  ...
  },
  {
    ...
    tips: "Mantén el cuerpo en línea recta.",
  ...
  },
  {
    ...
    tips: "Sube hasta la máxima contracción.",
    mecanicas: ["Empuje"],
    grupoMuscular: ["Tren Inferior"],
...
const CreateRoutineModal = ({
  ...
  // Initialize form when modal opens or rutina changes
  useEffect(() => {
    if (open) {
  ...
  // Calculate auto objective based on exercises
  const calculateAutoObjetivo = useMemo((): RutinaObjetivo => {
    const scores: RutinaObjetivo = {
      fuerza: 0,
      potencia: 0,
      agilidad: 0,
      coordinacion: 0,
    ...
    formData.bloques.forEach((bloque) => {
      ...
      bloque.ejercicios.forEach((ej) => {
        ...
        // Primary aptitudes weight more (2x)
  ...
  // Update auto objective when mode is auto
  useEffect(() => {
  ...
  const addBloque = () => {
    updateFormData("bloques", [...formData.bloques, createEmptyBloque()]);
  };

  const updateBloque = (index: number, bloque: RutinaBloque) => {
    const newBloques = [...formData.bloques];
    newBloques[index] = bloque;
    updateFormData("bloques", newBloques);
  };

  const deleteBloque = (index: number) => {
    updateFormData("bloques", formData.bloques.filter((_, i) => i !== index));
  };

  const moveBloque = (index: number, direction: "up" | "down") => {
    const newBloques = [...formData.bloques];
  ...
  const handleDragStart = (e: React.DragEvent, ejercicio: Ejercicio) => {
    setDraggedEjercicio(ejercicio);
  ...
  const handleDropOnBloque = (bloqueIndex: number) => {
    ...
    const newEjercicio: RutinaEjercicio = {
      ...
      ejercicio: draggedEjercicio,
      tipoEjecucion: "tiempo",
      tiempo: 30,
      repeticiones: 10,
    };
    ...
    toast({
      ...
      description: `"${draggedEjercicio.nombre}" agregado al bloque ${bloqueIndex + 1}`,
    });
  };
  ...
  const validateForPublish = (): string[] => {
    ...
    if (formData.bloques.length === 0) {
      errors.push("Debe contener al menos 1 bloque");
    }

    const totalEjercicios = formData.bloques.reduce((sum, b) => sum + b.ejercicios.length, 0);
    if (totalEjercicios === 0) {
      errors.push("Debe contener al menos 1 ejercicio");
    }

    formData.bloques.forEach((bloque, bi) => {
      bloque.ejercicios.forEach((ej, ei) => {
        if (ej.tipoEjecucion === "tiempo" && ej.tiempo <= 0) {
          errors.push(`Bloque ${bi + 1}, Ejercicio ${ei + 1}: tiempo debe ser > 0`);
        }
        if (ej.tipoEjecucion === "repeticiones" && ej.repeticiones <= 0) {
          errors.push(`Bloque ${bi + 1}, Ejercicio ${ei + 1}: repeticiones deben ser > 0`);
        }
      });

      if (bloque.repetirBloque && bloque.series < 2) {
        errors.push(`Bloque ${bi + 1}: si repite, series debe ser >= 2`);
      }
    });
  ...
  const handlePublish = () => {
    ...
    if (errors.length > 0) {
      toast({
        title: "Errores de validación",
        description: errors.slice(0, 3).join(". ") + (errors.length > 3 ? ` (+${errors.length - 3} más)` : ""),
        variant: "destructive",
      });
      return;
    }
  ...
  const totalTime = formData.bloques.reduce((bSum, bloque) => {
    const bloqueTime = bloque.ejercicios.reduce((eSum, ej) => {
      return eSum + (ej.tipoEjecucion === "tiempo" ? ej.tiempo : 30) + bloque.descansoEntreEjercicios;
    }, 0);
          ...
          <DialogHeader className="px-6 py-4 border-b border-border">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <Badge
                  variant={formData.estado === "publicada" ? "default" : "secondary"}
                  className="font-normal"
                >
                  {formData.estado === "publicada" ? "Publicada" : "Borrador"}
                </Badge>
              </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <ScrollArea className="flex-1">
                <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => updateFormData("categoria", v as Rutina["categoria"])}
                      >
                        <SelectTrigger className="bg-card border-border">
                          <SelectValue placeholder="Seleccionar categoría" />
                        </SelectTrigger>
                        <SelectContent>
                          {CATEGORIAS_RUTINA.map((cat) => (
                  ...
                  <ObjectiveScoring
                    ...
                    onModeChange={(mode) => {
                      updateFormData("objetivoMode", mode);
                      if (mode === "auto") {
                        updateFormData("objetivo", calculateAutoObjetivo);
                      }
                    }}
                  ...
                  <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                    ...
                    <div className="flex items-center gap-1">
                      <Input
                        type="number"
                        min={0}
                        value={formData.descansoEntreBloques}
                        onChange={(e) => updateFormData("descansoEntreBloques", parseInt(e.target.value) || 0)}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="outline" size="sm" onClick={addBloque} className="gap-1">
                    ...
                    {formData.bloques.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-border rounded-lg">
                        ...
                        <Button variant="outline" onClick={addBloque} className="gap-1">
                          <Plus className="h-4 w-4" />
                      ...
                      <div className="space-y-4">
                        {formData.bloques.map((bloque, index) => (
                          <BlockEditor
                            ...
                            bloque={bloque}
                            index={index}
                            onUpdate={(b) => updateBloque(index, b)}
                            onDelete={() => deleteBloque(index)}
                            onMoveUp={() => moveBloque(index, "up")}
                            onMoveDown={() => moveBloque(index, "down")}
                            canMoveUp={index > 0}
                            canMoveDown={index < formData.bloques.length - 1}
                            onDrop={() => handleDropOnBloque(index)}
                          />
                        ))}
                      </div>
                    )}
              ...
              <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-border bg-muted/20">
                <Button variant="ghost" onClick={handleClose} className="gap-1">
                  <X className="h-4 w-4" />
                  Cancelar
                </Button>
